...
| [`project-pulse`](./project-pulse/SKILL.md) | Project-scoped monitoring and telemetry for phased delivery. Adds timestamps, structured events, and an event log to `project_state.json` — enabling dashboards and cross-project visibility without slowing down the agent. | Sibling to `phase-tracking`. Use both for measurable delivery tracking, dashboards, alerts, and compliance. |
