---
name: project-pulse
description: Project-scoped monitoring and telemetry layer for phased delivery. Adds timestamps, structured events, and an event log to project_state.json — enabling dashboards and cross-project visibility without slowing down the agent.
license: MIT
metadata:
  author: sisyphus
  version: "1.0.0"
...
| Schema validation | `project-pulse` |
| Event log (append-only chronological record) | `project-pulse` |
...
After every `project_state.json` write, run the validation gate (see Health checks & validation section).
...
5. Mark the first todo `in_progress` and proceed.
...
6. If no phases remain, set `current_phase` to `"Complete"` and `current_phase_started_at` to null.
7. Update `next_steps` with the next phase goal.
8. Run full project verification (all tests, lint, typecheck).
...
The agent's only webhook responsibility is appending events to `event_log`.
Delivery to external systems (Slack, Datadog, custom dashboards) is handled by a
**sidecar process** — cron, inotify, or CI — that reads `project_state.json`,
detects new events, and POSTs them with HMAC-SHA256 signing.

This keeps HTTP IO, retries, and error handling out of the agent's critical path.

> See [`project-pulse-webhook-sidecar.md`](../../../Documents/ideas/project-pulse-webhook-sidecar.md)
> for the full sidecar pattern, including delivery scripts, retry policies, and
> cursor-based event tracking.

## Export & snapshot (external)

Exporting `project_state.json` to a collector, archive, or CI pipeline is also
handled **outside the agent**. Patterns include cron-based push, GitHub Actions
steps, and timestamped archive snapshots — all documented in the sidecar idea.

> See [`project-pulse-webhook-sidecar.md`](../../../Documents/ideas/project-pulse-webhook-sidecar.md)
> for export and delivery patterns.
...
// 2. project-pulse — manages monitoring and events
```
...
The full design exploration for monitoring dashboards, metrics, and visualization
tools is documented alongside the sidecar delivery pattern:

- **Dashboard & metrics design**: [`phase-tracking-monitoring-integration.md`](../../../Documents/ideas/phase-tracking-monitoring-integration.md)
- **Sidecar webhook delivery**: [`project-pulse-webhook-sidecar.md`](../../../Documents/ideas/project-pulse-webhook-sidecar.md)
